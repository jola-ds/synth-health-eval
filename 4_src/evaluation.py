...
def train_and_evaluate(X_train, y_train, X_test, y_test, model_name, base_model, param_grid):
    """
...
def run_master_loop(data_path, output_path, params_output_path=None, model_type='copula'):
    """
    ...
    for train_index, test_index in rskf.split(X, y):
        ...
        for name, (model, grid) in PREDICTORS.items():
            ...
            res_A = train_and_evaluate(X_train_real, y_train_real, X_test_real, y_test_real, name, model, grid)
            res_A.update({'Fold': fold_idx, 'Scenario': 'A_Baseline'})
            all_results.append(res_A)
            
            # Scenario B: Fidelity (Syn Train = Real Size)
            res_B = train_and_evaluate(X_syn_B, y_syn_B, X_test_real, y_test_real, name, model, grid)
            res_B.update({'Fold': fold_idx, 'Scenario': 'B_Fidelity'})
            all_results.append(res_B)
            
            # Scenario C: Scale (Syn Train = 1000)
            res_C = train_and_evaluate(X_syn_C, y_syn_C, X_test_real, y_test_real, name, model, grid)
            ...
            for ratio in ratios:
                ...
                X_aug = pd.concat([X_train_real, syn_aug.drop(columns=['hypertension'])])
                y_aug = pd.concat([y_train_real, syn_aug['hypertension']])
                
                res_D = train_and_evaluate(X_aug, y_aug, X_test_real, y_test_real, name, model, grid)
...
if __name__ == "__main__":
    ...
    output_path = "c:/Users/Moses Omotunde/Documents/Me/synth-health-eval/results_master_loop_params.csv"
    params_path = "c:/Users/Moses Omotunde/Documents/Me/synth-health-eval/results_params.csv"
    
    # Run with Copula (since it was the winner) to get the final params
    run_master_loop(data_path, output_path, params_output_path=params_path, model_type='copula')
