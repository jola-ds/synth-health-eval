...
def run_fidelity_check(data_path):
    ...
    continuous_cols = ['age', 'weight', 'height', 'pulse', 'systolic_bp', 'diastolic_bp']
    ...
    for col, res in ks_results.items():
        print(f"{col}: KS Stat = {res['KS_Stat']:.4f} (Lower is better), Score = {res['Score']:.4f}")
        
    print("\n--- 2. Multivariate Fidelity (Correlation) ---")
    frob_norm, _, _ = calculate_correlation_diff(df_real, df_syn)
    print(f"Correlation Matrix Difference (Frobenius Norm): {frob_norm:.4f} (Lower is better)")
    ...
    print("Interpretation: 0.5 = Perfect Fidelity (Indistinguishable), 1.0 = Poor Fidelity (Easy to spot)")
