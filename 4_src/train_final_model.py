...
def get_best_params(params_path, model_name='RF', scenario='D_Augment_0.5'):
    ...
    mode_row = subset_filled.groupby(param_cols).size().reset_index(name='count').sort_values('count', ascending=False).iloc[0]
    
    print(f"Most frequent parameters for {model_name} ({scenario}) found in {mode_row['count']} folds:")
    best_params = {}
...
def train_final_model(data_path, params_path, output_model_path):
    ...
    print(f"Final Training Set: {len(X_final)} samples (Real: {len(X_real)}, Syn: {len(X_syn)})")
...
if __name__ == "__main__":
    ...
    output_model_path = "c:/Users/Moses Omotunde/Documents/Me/synth-health-eval/final_model_rf.pkl"
